...
test = pd.read_csv('test_2019_10_june_final.csv', dtype=dictionary)
...
#index_test = to_be_trained_keys.index('208_777')
...
for key in to_be_trained_keys:
    ...
    if (len(train_dfs[key].index) >= 5):
        ...
        if(len(train_dfs[key].index) > 20 and len(test_dfs[key].index) > 20):
          pca = PCA(n_components=20)
        else:
          pca = PCA(n_components=min(len(train_dfs[key].index),len(test_dfs[key].index)))
          print("Number of Principle Components:" + str(min(len(train_dfs[key].index),len(test_dfs[key].index))))
        ...
        features_train = pca.n_components_
        features_test = pca.n_components_
        
        

        supervised_train.drop(supervised_train.columns[range(n_pca_components+1, (n_pca_components*2)+0)], axis=1, inplace=True)
        supervised_train.drop(supervised_train.columns[range(n_pca_components+2, (n_pca_components*2)+1)], axis=1, inplace=True)
        supervised_train.drop(supervised_train.columns[range(n_pca_components+3, (n_pca_components*2)+2)], axis=1, inplace=True)
        supervised_train.drop(supervised_train.columns[range(n_pca_components+4, (n_pca_components*2)+3)], axis=1, inplace=True)

        supervised_test.drop(supervised_test.columns[range(n_pca_components+1, (n_pca_components*2)+0)], axis=1, inplace=True)
        supervised_test.drop(supervised_test.columns[range(n_pca_components+2, (n_pca_components*2)+1)], axis=1, inplace=True)
        supervised_test.drop(supervised_test.columns[range(n_pca_components+3, (n_pca_components*2)+2)], axis=1, inplace=True)
        supervised_test.drop(supervised_test.columns[range(n_pca_components+4, (n_pca_components*2)+3)], axis=1, inplace=True)
        ...
        for i in range(0, ahead):
            ...
            inv_new = np.concatenate((y_pred_i, X_test[:, -(n_pca_components-1):]), axis=1)
        ...
        for index, row in test_df_truncated.iterrows():
            ...
            for j in range(0, ahead):
                ...
                temp_df.loc[i + j] = [key.split('_')[0]] + [key.split('_')[1]] + [str(index)] + [str(j)] + [str(y_predicted[j][k])]
            i += 4
        result_df = result_df.append(temp_df)

print(trained_keys)

result_df.to_csv('results_test.csv', encoding='utf-8', index=False)
