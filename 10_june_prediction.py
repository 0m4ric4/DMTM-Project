...
to_be_trained_keys = to_be_trained_keys[350:400]
...
for key in to_be_trained_keys:
    ...
    if (len(train_dfs[key].index) >= 5):
        ...
        supervised_train.drop(supervised_train.columns[range(n_pca_components + 1, (n_pca_components * 2) + 0)], axis=1,
                              inplace=True)
        supervised_train.drop(supervised_train.columns[range(n_pca_components + 2, (n_pca_components * 2) + 1)], axis=1,
                              inplace=True)
        supervised_train.drop(supervised_train.columns[range(n_pca_components + 3, (n_pca_components * 2) + 2)], axis=1,
                              inplace=True)
        supervised_train.drop(supervised_train.columns[range(n_pca_components + 4, (n_pca_components * 2) + 3)], axis=1,
                              inplace=True)

        supervised_test.drop(supervised_test.columns[range(n_pca_components + 1, (n_pca_components * 2) + 0)], axis=1,
                             inplace=True)
        supervised_test.drop(supervised_test.columns[range(n_pca_components + 2, (n_pca_components * 2) + 1)], axis=1,
                             inplace=True)
        supervised_test.drop(supervised_test.columns[range(n_pca_components + 3, (n_pca_components * 2) + 2)], axis=1,
                             inplace=True)
        supervised_test.drop(supervised_test.columns[range(n_pca_components + 4, (n_pca_components * 2) + 3)], axis=1,
                             inplace=True)
        ...
        if(len(y_pred)==0):
            ...
            for index, row in test_df_truncated.iterrows():
                ...
                for j in range(0, ahead):
                    ...
                    temp_df.loc[i + j] = [key.split('_')[0]] + [key.split('_')[1]] + [str(index)] + [str(j)] + [
                        str(y_predicted[j][k])]
                i += 4
...
result_df.to_csv('results_350_399.csv', encoding='utf-8', index=False)
